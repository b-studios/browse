...
function addInteractivity(dom) {
	...
	$dom.find("code [data-defs]").live("mouseover",
	...
	$dom.find("code a[href]").live("mouseover",
		function() {console.log("Hovered over", this);  $(this).definition().highlight() }
	).live("mouseout",
...
function searchBar() {
  ...
  var search = $("<input>")
    .attr("type", "search")
    .attr("placeholder", "search for files...")
    .appendTo(nav);
...
jQuery.fn.extend({
	...
	definition: function () {
	  var ref = $(this).attr("data-ref-id");
	  var target = $("[data-def-ids~='" + ref + "']");
	  return target ? $(target) : $(this);
	},
	
	references: function () {
	  var refs = [];
	  $(this.attr('data-def-ids').split(" ")).each(function (i, el) {
	    [].push.apply(refs, $("[data-ref-id='" + el +"']"))
	  });
	  return $(refs);
	},
	
	
	/* Old version - used name-strings to find references
	definition: function() {
	  ...
	  // TODO check whether on correct page 
	
		var target = $("[data-defs~='" + ref + "']");
		return target ? $(target) : $(this);
	},
	references: function() {
	  var refs = [];
	  $(this.attr('data-defs').split(" ")).each(function (i, el) {
	    [].push.apply(refs, $("a[href$='#" + el +"']"))
	  });	
	  return $(refs);
	},
